...
comparison. This is simply a matter of instantiating a `umap.umap_.UMAP` object (in
...
`umap.umap_.UMAP.fit_transform` method with the data we wish to reduce. By default
...
are familiar with the sklearn API you'll know that the `umap.umap_.UMAP.fit` method
...
model). We can simply pass the `umap.umap_.UMAP` model that target data when
...
Next we can use the `umap.umap_.UMAP.transform` method on that model to transform the
...
the `umap.umap_.UMAP.transform` method.
...
The [Galaxy10SDSS dataset](https://astronn.readthedocs.io/en/latest/galaxy10sdss.html)
...
[Galaxy 10 DECals dataset](https://astronn.readthedocs.io/en/latest/galaxy10.html)
